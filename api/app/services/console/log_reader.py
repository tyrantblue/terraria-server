...
class LogReader:
    ...
    def read_lines(self, offset: int) -> tuple[int, list[str]]:
        """从 offset 开始读出所有完整行，返回 (新偏移, 行文本列表)。"""
        size = self.size()
        if size <= offset:
            return offset, []
        ...
        chunk = data[: cut + 1]
        lines = [raw.decode("utf-8", errors="replace") for raw in chunk.split(b"\n")[:-1]]
        return offset + len(chunk), lines

    # -- 尾部读取（/api/server/console 用） ---------------------------
    def tail(self, count: int, max_bytes: int = TAIL_MAX_BYTES) -> list[str]:
        """返回最后 count 行文本，只回读文件尾部至多 max_bytes 字节。"""
        if count <= 0:
            return []
        size = self.size()
        ...
        if cut == -1:
            return []
        lines = [raw.decode("utf-8", errors="replace") for raw in data[: cut + 1].split(b"\n")[:-1]]
        return lines[-count:]
